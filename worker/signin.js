...
//eto yung javascript na mahalaga para sa database
document.addEventListener("DOMContentLoaded", () => { //yung DOMContentLoaded para lang pag nag load yung sign in page ang gagawin nya ay ioopen nya yung database
    ...
    const loginForm = document.querySelector("#login"); //ano yung form na may login na id  which is nasa html yung # means id
 
    loginForm.addEventListener("submit", e => {
        e.preventDefault();
        var inputs = loginForm.getElementsByTagName("input"); //dun sa form kanina kukunin yung mga input
        ...
        //ito yung nag cconnect sa database natin kaya naka lagay ay db.collection
        ...
        db.collection(table).doc({ email: username.value, password:password.value }).get().then(document => { 
            ...
            if(document === undefined){ //undefined wala syang kamatch dito mag eerror
